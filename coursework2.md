...
2.  Create a _private_ repository called e.g. `bingyulab/sustainable-scientific-software-homework`.

3.  Clone your repository. In a terminal run, e.g.: `git clone git@github.com:bingyulab/sustainable-scientific-software-homework.git` substituting with the correct location of your repository.
...
1. `git add` and `git push` the file `Dockerfile` to the repository.
...
1. Start a Docker container using your image and share your repository into a    directory `/root/shared` into the container.
	```python
	docker run -it -v $(pwd):/root/shared bingyujiang/sss
	```

2. Run the tests inside the container by going to `/root/shared` and running the    command `pytest`. The tests should fail.
````bash
cd /root/shared
pytest
````
3. In a terminal on the host modify `wallet.py` until the tests in `test_wallet.py` all pass.
  ...
  test:
    ...
    steps:
      ...
      - name: Run tests using Docker
        run: |
          docker pull bingyujiang/sss
          docker run -v ${{ github.workspace }}:/root/shared bingyujiang/sss bash -c "cd /root/shared && pytest"
```
